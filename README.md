...
- **CI**：修改 `inbox/` 或校验脚本时，GitHub Actions 会运行 **`inbox-frontmatter-check`**（工作流显示名）；本地可 `pip install -r requirements-ci.txt` 后执行 `python3 scripts/validate_inbox_frontmatter.py`。  
