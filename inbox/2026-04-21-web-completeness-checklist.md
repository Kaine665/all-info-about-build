...
原文件曾置于本仓库 `docs/web-completeness-checklist.md`。已明确 **`docs/` 仅用于 all-info-about-build 自身的元文档**（维护本仓库的说明），不用于存放与「构建信息收集」主题无关的通用清单。该网页检查表属于可复用知识，故迁入 `inbox/`。若日后吸收为正式内容，目标应为根目录 `README` / `SPEC` 或团队飞书/外部知识库，**而非**本仓库的 `docs/`（除非将来明确将本仓库扩展为含此类附录）。
...
1. **新项目启动**：复制到目标项目的 `docs/` 或内部 wiki，按项目类型删除不适用项。
