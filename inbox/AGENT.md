...
`inbox/` 只作**收件箱**：新信息默认**新建一篇** `YYYY-MM-DD-<主题短名>.md` 即可，**不在此目录**做「同一主题合并到旧文件」或目录级去重；去重、归类、正式结构在迁入 **根目录 `README.md` / `SPEC.md` / 外部知识库** 等正式位置时由人类流程完成。本仓库的 **`docs/` 仅用于 all-info-about-build 元文档**，不是收集体知识的默认落点（见 [`docs/README.md`](../docs/README.md)）。
...
`inbox/` 不是规范单一真相源；吸收进根目录 `README.md`、`SPEC.md` 或外部知识库后，将对应条目的 `status` 改为 **`absorbed`**，并在 **`## 吸收说明`** 中写明目标路径与章节。若吸收目标仅为本仓库元文档，再写 `docs/` 下具体文件。
